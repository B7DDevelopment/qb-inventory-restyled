...
RegisterNUICallback("requestPlayerInfo", function(_, cb)
    ...
    if playerData.charinfo then
        fullname = (playerData.charinfo.firstname or "Unknown") .. " " .. (playerData.charinfo.lastname or "")
    end

    local phone = "No Phone"
    if playerData.charinfo and playerData.charinfo.phone then
        phone = playerData.charinfo.phone
    elseif playerData.metadata and playerData.metadata.phone then
        phone = playerData.metadata.phone
    end
    ...
    if playerData.job then
        ...
        if playerData.job.grade then
            jobGrade = playerData.job.grade.name or "None"
        end
    end
    ...
    cb({
        ...
        job = jobLabel,
        jobgrade = jobGrade,
        cash = "$" .. tostring(cashAmount),
        bank = "$" .. tostring(bankAmount),
        serverid = tostring(GetPlayerServerId(PlayerId())),
    })
end)
...
local function closeAnim()
    ...
    local ped = PlayerPedId()
    LoadAnimDict('anim@heists@ornate_bank@grab_cash')
    TaskPlayAnim(ped, 'anim@heists@ornate_bank@grab_cash', 'exit', 8.0, 8.0, 800, 49, 0, false, false, false)
    Wait(500)
    ClearPedTasks(ped)
end
...
RegisterNetEvent('inventory:client:AddDropItem', function(dropId, player, coords)
    ...
    print("^2[DEBUG] Final coords - x:", x, "y:", y, "z:", z)
    ...
    if Config and Config.UseItemDrop and Config.ItemDropObject then
        ...
        if GetResourceState('qb-target') == 'started' then
            ...
            print("^1[DEBUG] qb-target not available")
        end
        
        print("^2[DEBUG] Drop setup complete!")
    end
end)
...
RegisterNetEvent('inventory:client:RemoveDropItem', function(dropId)
    ...
    if Config and Config.UseItemDrop then
...
RegisterNUICallback("DropItem", function(data, cb)
    ...
    local amount = tonumber(data.amount) or itemData.amount
    if itemData and itemData.slot then
        TriggerServerEvent("inventory:server:DropItem", itemData, amount)
    end
    cb('ok')
end)
...
RegisterNUICallback('OpenBinInventory', function(data, cb)
    ...
    local binId = "PlayerBin_"..playerCitizenId
...
RegisterNUICallback('OpenInventory', function(data, cb)
    if data.inventory == "stash" and data.inventoryId then
        TriggerServerEvent("inventory:server:OpenInventory", "stash", data.inventoryId, {
            maxweight = data.maxweight or 100000,
            slots = data.slots or 30,
        })
        TriggerEvent("inventory:client:SetCurrentStash", data.inventoryId)
    end
    cb('ok')
end)
                    ...
                    action = function()
                        local authorizedItems = {
                            ["vending"] = {
                                ...
                                ["itemList"] = {
                                    ["General"] = Config.VendingCola
                                }
                            }
                        }
                        exports['qb-shops']:OpenShop(authorizedItems["vending"])
                    end,
...
local Sprunk = {
    [1] = vector3(434.02682495117, -985.6787109375, 35.963325500488)
}
local Cola = {
    [1] = vector3(435.05258178711, -985.69000244141, 35.895309448242)
}
local CandyBox = {
    [1] = vector3(436.05667114258, -985.53991699219, 35.92658996582),
    [2] = vector3(-353.15405273438, -143.71775817871, 39.254688262939)
}
